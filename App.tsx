    ...
    <ThemeProvider>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans transition-colors">
        ...
        <main className="container mx-auto p-4 md:p-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
              <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">Tasks for Today</h2>
              <TaskInput 
                onAddTask={addTask} 
              ...
              />
              {isLoading ? (
                   <div className="text-center py-16"><p className="text-slate-500 dark:text-slate-400">Loading tasks...</p></div>
              ) : (
        ...
        {isAddCategoryModalOpen && (
          ...
          >
            <div 
              className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 max-w-sm w-full transition-colors"
              onClick={e => e.stopPropagation()}
            >
              <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">Add New Category</h3>
              <div className="space-y-4">
                 <div>
                    <label htmlFor="category-name" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Category Name</label>
                    <input
                      type="text"
                      id="category-name"
                      ...
                      className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 transition-colors"
                    />
                 </div>
                 <div>
                    <label htmlFor="category-emoji" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Emoji (optional)</label>
                    <input
                      type="text"
                      id="category-emoji"
              ...
              <div className="mt-6 flex gap-3">
                <button
                  onClick={() => setIsAddCategoryModalOpen(false)}
                  className="w-full bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold py-2 px-4 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 dark:focus:ring-slate-500"
                >
                  Cancel
                </button>
                <button
